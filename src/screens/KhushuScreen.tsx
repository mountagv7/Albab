...
function KhushuToggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  ...
  const bgColor   = anim.interpolate({ inputRange: [0, 1], outputRange: ['#2a2a2a', Colors.gold] });
    ...
    <View>
      <View style={styles.sliderHeader}>
        ...
        <View style={styles.sliderPill}>
          <Text style={styles.sliderPillText}>{value} {unit}</Text>
        </View>
      </View>
      <ScrollView
        horizontal
      ...
      >
        {steps.map(s => (
          <TouchableOpacity
            key={s}
            onPress={() => onChange(s)}
            style={[styles.stepBtn, s === value && styles.stepBtnActive]}
          >
            <Text style={[styles.stepText, s === value && styles.stepTextActive]}>
              {s}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <SafeAreaView edges={['top']} style={{ flex: 1 }}>
        ...
        >
          ...
          <View style={styles.paywallHero}>
            <Text style={styles.premiumTag}>FONCTIONNALITÉ PREMIUM</Text>
            <Text style={styles.paywallLock}>🔒</Text>
            <Text style={styles.paywallTitle}>Mode Khushu</Text>
            <Text style={styles.paywallAr}>خشوع</Text>
...
function ConfigScreen({ onBack }: { onBack: () => void }) {
  ...
  const base = mH * 60 + mM;
  const timeline = [
    { dot: Colors.gold,  text: '🔒 Blocage démarre',       time: fmtMin(base - before) },
  ...
  if (saved) {
    return (
      <View style={[styles.container, styles.successCenter]}>
        ...
        <Text style={styles.successAr}>بسم الله</Text>
      </View>
    );
  }
      ...
      <SafeAreaView edges={['top']} style={{ flex: 1 }}>
        ...
        >
          ...
          <View style={styles.configHeader}>
            ...
            <Text style={styles.configAr}>خشوع</Text>
          </View>

          {/* Master toggle */}
          <View style={[styles.card, enabled && styles.cardGoldBorder]}>
...
const styles = StyleSheet.create({
  ...
  premiumTag: {
    fontSize: 11, color: Colors.gold, letterSpacing: 1, marginBottom: 20,
  },
  paywallLock: { fontSize: 52, marginBottom: 16 },
  paywallTitle: {
    fontSize: 26, fontWeight: '700', color: Colors.textPrimary, marginBottom: 6,
  },
  paywallAr: {
    fontSize: 26, color: Colors.gold, opacity: 0.7, marginBottom: 16,
  },
  paywallDesc: {
  ...
  ctaHint: { textAlign: 'center', fontSize: 12, color: Colors.textMuted },
  ...
  configAr: {
    fontSize: 22, color: Colors.gold, opacity: 0.6, marginTop: 2,
  },
  ...
  cardGoldBorder: { borderColor: 'rgba(201,168,76,0.3)' },
  ...
  sliderLabel: { fontSize: 11, color: Colors.textMuted, letterSpacing: 0.8 },
  sliderPill: {
    backgroundColor: Colors.goldBg,
    borderWidth: 1, borderColor: 'rgba(201,168,76,0.3)',
    borderRadius: 20, paddingHorizontal: 12, paddingVertical: 4,
  },
  sliderPillText: { fontSize: 14, fontWeight: '700', color: Colors.gold },
  ...
  stepTextActive: { color: Colors.gold },
  sliderMinMax: {
  ...
  timelineCard: {
    ...
    borderRadius: 18, padding: 19, marginBottom: 14,
  },
  sectionLabel: {
  ...
  prayerRow: {
    ...
    gap: 14, padding: 12, borderRadius: 12, borderWidth: 1,
  },
  prayerRowActive: {
  ...
  prayerRowInactive: {
    backgroundColor: '#0d0d0d', borderColor: '#1a1a1a',
  },
  ...
  saveBtnDisabled: {
    borderRadius: 16, paddingVertical: 19, alignItems: 'center',
    backgroundColor: '#1a1a1a', borderWidth: 1, borderColor: Colors.border,
    marginBottom: 14,
  },
  saveBtnTextDark: { fontSize: 16, fontWeight: '700', color: '#000' },
  saveBtnTextMuted: { fontSize: 16, fontWeight: '700', color: Colors.textMuted },
  saveHint: {
    textAlign: 'center', fontSize: 12, color: Colors.textMuted, lineHeight: 19,
  },
  ...
  successAr: { fontSize: 24, color: Colors.gold, opacity: 0.7 },
});
